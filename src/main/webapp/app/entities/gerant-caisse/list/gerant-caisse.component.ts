...
import { EntityArrayResponseType, GerantCaisseService } from '../service/gerant-caisse.service';
import { GerantCaisseDeleteDialogComponent } from '../delete/gerant-caisse-delete-dialog.component';
...
export class GerantCaisseComponent implements OnInit {
  ...
  trackId = (_index: number, item: IGerantCaisse): number => this.gerantCaisseService.getGerantCaisseIdentifier(item);
  ...
  delete(gerantCaisse: IGerantCaisse): void {
    const modalRef = this.modalService.open(GerantCaisseDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.gerantCaisse = gerantCaisse;
  ...
  protected fillComponentAttributesFromResponseBody(data: IGerantCaisse[] | null): IGerantCaisse[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.gerantCaisseService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
